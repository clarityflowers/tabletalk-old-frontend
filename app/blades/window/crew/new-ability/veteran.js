...
class Veteran extends React.PureComponent {
  render() {
    const { abilities, playbook, library, off, onAdd } = this.props;
    const abilityLists = [];
    ...
    for (let i=0; i < playbooks.length; i++) {
      ...
      if (playbookName != playbook) {
        ...
        const abilityList = listAbilities(names, abilities, library.def, onAdd);
        abilityLists.push(
          <Container key={i}>
...
const { array, string, object, bool, func } = React.PropTypes;
Veteran.propTypes = {
